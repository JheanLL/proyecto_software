...
export default function NewEmployeeForm({ areas }: NewEmployeeFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const anoNacimiento = new Date(formData.get("fechaNac") as string).getFullYear();
    if (new Date().getFullYear() - anoNacimiento < 18) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface border border-border rounded-2xl shadow-card p-6 md:p-8">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        {/* Fecha de nacimiento ahora tiene un defaultValue lógico */}
        {fechaNacDefault && (
